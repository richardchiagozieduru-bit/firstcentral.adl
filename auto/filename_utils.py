...
def generate_filename_from_user(subscriber_id, subscriber_name, file_type, borrower_type, assigned_month=None, assigned_year=None):
    """
    Generate standardized filename using user's assigned subscriber information
    
    Args:
        subscriber_id (str): User's assigned subscriber ID
        subscriber_name (str): User's assigned subscriber name
        file_type (str): 'excel' or 'txt'
        borrower_type (str): 'consumer' or 'commercial'
        assigned_month (int, optional): Assigned month (1-12)
        assigned_year (int, optional): Assigned year
    ...
    """
    # Ensure subscriber_id is an integer (remove any decimal formatting)
    try:
        subscriber_id = str(int(float(subscriber_id)))
    except (ValueError, TypeError):
        # If conversion fails, use as-is but strip any whitespace
    ...
    # Remove spaces from subscriber name for filename
    ...
    filename = f"{subscriber_id}_{date_string}_{type_digit}_{subscriber_name_clean}_{month_name}_{year_string}_{borrower_type_str}{file_extension}"
...
def generate_filename(alias, file_type, borrower_type, assigned_month=None, assigned_year=None):
    ...
    # Construct filename according to specification
    # Format: SUBID_YYYYMMDD_TypeDigit_SUBSCRIBERNAME_Month_Year_BorrowerType.extension
    filename = f"{subid}_{date_string}_{type_digit}_{subscriber_name_clean}_{month_name}_{year_string}_{borrower_type_str}{file_extension}"
